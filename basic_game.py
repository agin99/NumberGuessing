...
def take_yn_input(game_connection, query_string):
    while True:
        game_connection.send(f"1,{query_string}".encode('utf-8'))
        ...
        try:
            ...
            game_connection.send("Must choose y or n.".encode('utf-8'))
        

def guessing_game(IPv4, port):
    #Connect game logic to game server
    game_connection = connect_client_to_game(IPv4, port)
    ...
    trigger = input("Stall, click any key to continue.")
    game_continuation = take_yn_input(game_connection, "Continue on previous account? (y/n)")
    game_connection.close()
    
    # game_connection.send("Send your clientID".encode('utf-8'))
    # data = game_connection.recv(1024).decode('utf-8')
    # if not data:
    #     print("Server shutdown.")
    #     break
    # else: 
    ...
    #         restart_inquiry = take_yn_input(game_connection, "Would you like to continue your current game? (y/n)")
